...
showData.addEventListener("click",function(){
  ...
  .domain([d3.min(stockDatas,d=>parseInt(d.transactVolume.replace(/,/g, ""))),d3.max(stockDatas,d=>parseInt(d.transactVolume.replace(/,/g, "")))]).range([innerHeight,0]);
  ...
  xAxis.ticks(stockDatas.length).tickFormat(dateFormatter);

  g.append('g').attr('transform',`translate(0,${innerHeight})`).call(xAxis);

  console.log(g);

  const yRect=d3.scaleLinear().domain([d3.min(stockDatas, d => parseInt(d.transactVolume.replace(/,/g, ""))), d3.max(stockDatas, d => parseInt(d.transactVolume.replace(/,/g, "")))]).range([innerHeight,0]);

  const rects=g.selectAll('rect').data(stockDatas).enter().append('rect').attr('x',(d,i)=>{return xScale(convertRocToDate(d.date))})
  .attr('width',1)
